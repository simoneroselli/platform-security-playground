...
| **Container Hardening** | [Read-Only Filesystem & Volatile Storage](./container-hardening/06-readonly-filesystem/README.md) | Structuring runtime immutability via `tmpfs`, stripping native binary execution capabilities via `noexec` flags, and handling K8s CSI mount limitations using the Init Container remount trick. |
